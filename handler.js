          ...
          if (chat.welcome) {
            ...
            for (let user of participants) {
              ...
              text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'ようこそ Youkuso, @user!').replace('@subject', this.getName(jid)).replace('@desc', groupMetadata.desc) :
                  (chat.sBye || this.bye || conn.bye || '左様なら Sayounara, @user! ,KALAU BALIK JANGAN LUPA BAWA GORENGAN YAH KAK 😆')).replace(/@user/g, '@' + user.split`@`[0])
                let wel = `━━━━━━ 𝗪𝗘𝗟𝗖𝗢𝗠𝗘 𝗞𝗔𝗞 👋🏻 ━━━━━━`
                let lea = `━━━━━━ 𝗚𝗢𝗢𝗗 𝗕𝗬𝗘 𝗕𝗘𝗕𝗔𝗡 😆🖕🏻 ━━━━━━`
                this.reply(jid, text, 0, { thumbnail: kai, contextInfo: {
                mentionedJid: [user],
                externalAdReply: {
                  mediaUrl: 'https://instagram.com/jojo_wibu',
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    botAdmin: '*BOT AJA BUKAN ADMIN ANJENG😑*',
